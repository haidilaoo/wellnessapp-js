...
const updateProfileImageInPosts = async (newProfileImage, userUid) => {
    ...
    postsSnapshot.forEach((docSnapshot) => {
        ...
        batch.update(postRef, { profileImageUri: newProfileImage }); // Update the nickname in each post
    });

    // Commit the batch update
    await batch.commit();

    console.log("Posts updated with new profile image.");
};

export { updateProfileImageInPosts };
